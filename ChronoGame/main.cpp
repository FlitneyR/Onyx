...
int main( int argc, const char** argv )
{
...

	{
		...
		pre_tick_set.AddSubset( chrono::Physics::UpdatePhysicsBodies, chrono::Physics::UpdateCollisions );
		pre_tick_set.AddSystem( chrono::Core::UpdateCamera );
		tick_set.AddSystem( chrono::Core::UpdateOffScreenSpawners );
		tick_set.AddSystem( chrono::Physics::UpdateDamageOnCollision );
		tick_set.AddSystem( chrono::Player::UpdatePlayers );
		...
		post_tick_set.AddSystem( chrono::Core::UpdateLifetimes );
		...
		if ( std::shared_ptr< onyx::IWindow > game_window = WEAK_ASSERT( window_manager.CreateWindow( create_window_args ), "Failed to create game window" ) )
		{
			...
			while ( !window_manager.WantsToQuit() )
			{
				...
				pre_tick_set.Run( world, cmd, chrono_asset_manager, tick_data, camera, rng );
				tick_set.Run( world, cmd, chrono_asset_manager, tick_data, camera, rng );
				post_tick_set.Run( world, cmd, chrono_asset_manager, tick_data, camera, rng );
