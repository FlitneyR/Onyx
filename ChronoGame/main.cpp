...

		{
			...
			if ( std::shared_ptr< onyx::IWindow > game_window = WEAK_ASSERT( window_manager.CreateWindow( create_window_args ), "Failed to create game window" ) )
			{
				...
				while ( !window_manager.WantsToQuit() )
				{
					...
					pre_tick_set.Run( world, cmd, chrono_asset_manager, tick_data, camera );
					tick_set.Run( world, cmd, chrono_asset_manager, tick_data, camera );
					post_tick_set.Run( world, cmd, chrono_asset_manager, tick_data, camera );
